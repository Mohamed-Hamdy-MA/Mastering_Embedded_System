 ...
 * @example
 * uint8_t position;
 * position = getConfigPositionInReg(GPIO_PinNumber_0);
 * // position will be 0
 */
uint8_t getConfigPositionInReg(uint16_t PinNumber)
{
	switch (PinNumber)
	{
...
void MCAL_GPIO_InitPin(GPIOx_REG_t* GPIOx, GPIO_PinConfig_t* PinConfig)
{
	...
	ConfigPositionInReg = getConfigPositionInReg(PinConfig->GPIO_PinNumber);

	//Clear these 4 configuration bits [CNFx1 CNFx0 MODEx1 MODEx0] related to PinNumber x
	*ConfigReg &= ~(0xF << ConfigPositionInReg);
	...
	case GPIO_PinMode_Input_FLOATING:		 // [CNFx1 CNFx0 MODEx1 MODEx0] = [0 1 0 0]
		*ConfigReg |= (0b0100 << (ConfigPositionInReg));
		break;

	case GPIO_PinMode_Input_PULLUP:		 // [CNFx1 CNFx0 MODEx1 MODEx0] = [1 0 0 0] & [ODRx] = [1]
		*ConfigReg |= (0b1000 << (ConfigPositionInReg));
